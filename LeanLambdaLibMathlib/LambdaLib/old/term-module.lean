...
public inductive Constant
| strConst : String -> Constant

public inductive Term : Type
| var : Nat → Term
| const : Constant → Term
...
public def lift (k : Nat) (t : Term) : Term :=
  match t with
...
public def subst (k : Nat) (toSub : Term) (t : Term) : Term :=
  match t with
...
public inductive Step : Term → Term → Prop where
| app1 : ∀{L L' M : Term},
    Step L L'
...
--------------------------------------------------------------------------------
@[expose]
public def Relation (A : Type) : Type := A → A → Prop

public def closeRef {A} (R : Relation A) : Relation A :=
  fun x y => x = y ∨ R x y

public def liftRef {A} {B} {R : Relation A} {R' : Relation B} {x y : A} (f : A → B)
...
public inductive closure {A} (R : Relation A) : A → A → Prop
| refl : ∀{a : A}, closure R a a
| cons : ∀{x y z : A}, R x y → closure R y z  → closure R x z

public def oneStep {A} {R : Relation A} {x y : A} (step : R x y)
  : closure R x y := closure.cons step closure.refl

public def transitivity {A} {R : Relation A} {x y z : A}
...
public def closureClosure {A} {R : Relation A} {x y : A} (H : closure (closure R) x y)
  : closure R x y :=
  match H with
  | .refl => closure.refl
  | .cons a b => transitivity a (closureClosure b)

public def liftCsr {A} {B} {R : Relation A} {R' : Relation B} {x y : A} (f : A → B)
  (ctr : ∀ {x y}, R x y → R' (f x) (f y))
...
public def liftCsr2 {A} {B} {R : Relation A} {R' : Relation B} {x y x' y' : A} (f : A → A → B)
...
public inductive union {A} (R S : Relation A) : A → A → Prop
| r : ∀{x y}, R x y → union R S x y
| s : ∀{x y}, S x y → union R S x y

public def liftUnion {A} {B} {R S : Relation A} {R' S' : Relation B} {x y : A} (f : A → B)
...
@[expose] public def square {A} (R S T U : Relation A) : Prop :=
  {x y z : A} → R x y → S x z → ∃ u, T y u ∧ U z u

@[expose] public def commute {A} (R S : Relation A) : Prop := square R S S R
@[expose] public def diamond {A} (R : Relation A) : Prop := commute R R
@[expose] public def confluent {A} (R : Relation A) : Prop := diamond (closure R)
...
public inductive StepEta : Term → Term → Prop where
| app1 : ∀ {L L' M : Term},
    StepEta L L'
...
@[expose]
public def AllStep : Relation Term := closure (union Step StepEta)

public theorem confluence : confluent (union Step StepEta) :=
...
@[expose]
public def equiv (t1 t2 : Term) := ∃ t, AllStep t1 t /\ AllStep t2 t

-- lifts from empty context to any context
public def liftMulti (n : Nat) (t : Term) : Term :=
  match n with
  | .zero => t
  | .succ n' => lift 0 (liftMulti n' t)

public theorem liftMultiStep {i} {M M' : Term}
  (step : Step M M')
...
public theorem liftMultiStepEta {i} {M M' : Term}
  (step : StepEta M M')
...
public theorem liftLiftMulti (n i : Nat) (H : i ≤ n) (t : Term)
