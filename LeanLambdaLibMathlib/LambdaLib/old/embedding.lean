...
| lambda : ∀{ctx A B s lvl},
  Typed ctx (Nat.succ lvl) S.U A
  → Typed <{S.cons} {ctx} {const (.natConst lvl)} {A}> (Nat.succ lvl) S.U B
...
theorem fundamental_lemma {ctx T lvl t env}
  ...
  cases mT with
  | lambda tyA tyB body =>
    have ⟨mtyA, mtyAeq, bla⟩ := fundamental_lemma tyA mctx
    -- have ⟨mty, mtyeq, bla⟩ := fundamental_lemma tyB mctx
    --
    repeat (first
      | simp only [means, means', means_prog] at mtyAeq
      | simp only [runProg, runProgDefinitionRet, runProgDefinitionRec, collectOptionDef] at mtyAeq
      | simp (disch := clear mtyAeq; intros; lambda_solve; try trivial) only
        [PmatchDef1, PmatchDef2, Pmatch2Def1, Pmatch2Def2, PifDef1, PifDef2] at mtyAeq
    )
    simp at mtyAeq
    subst mtyA
    --
