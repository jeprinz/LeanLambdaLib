...
def qSucc {n} : QSubst n (succ n) :=
  Quotient.mk SSubst (renToSub Var.succ)
...
theorem ruleCancelWeakenSubstZero {n} {t : QTerm n}
  {t2 : QTerm n}
  : qsubst (qSubstZero t2) (qsubst qSucc t) = t := by
    revert t
    apply Quotient.ind
    intro t
    revert t2
...
theorem ruleSubVarSuc {n1 n2} {i : Var n1}
  {sub : QSubst n1 n2}
  : qsubst (qexts sub) (qvar (Var.succ i)) = qsubst qSucc (qsubst sub (qvar i)) := by
    revert sub
    apply Quotient.ind
    intro sub
    simp [qsubst, Quotient.lift₂, Quotient.lift, Quotient.mk, Quot.lift, qexts, qvar, qSucc, subst
      , renToSub, subst]
    apply congrArg
    simp [exts, ids]
    rw [renameSubstRen]
...
theorem exampleProof2 {X : QTerm zero}
  : qapp (qlam (qlam (qvar (Var.succ Var.zero)))) X = qlam (qsubst qSucc X) := by
  rw [ruleBeta]
  rw [ruleSubstLam]
  rw [ruleSubVarSuc]
  rw [ruleSubVarZero]
...
macro_rules
  | `(tactic| simplify) => `(tactic|
    repeat (first
      | rw [ruleSubstApp] at *
      | rw [ruleSubstLam] at *
      | rw [ruleSubstVar] at *
      | rw [ruleCancelWeakenSubstZero] at *
      | rw [ruleSubVarZero] at *
      | rw [ruleSubVarSuc] at *
      | rw [ruleBeta] at *
     )
    )

theorem exampleProof4
  : @qapp zero (qlam (qlam (qvar (Var.succ Var.zero)))) (qlam (qvar Var.zero))
    = qlam (qlam (qvar Var.zero)) := by
      simplify
      rfl
